...
class LeadTaskServiceImpl(
    ...
    override fun getAllTasks(paginationRequest: PaginationRequest): PaginatedResponse<LeadTasksResponse> {
        return try {
            ...
            val leadTasksResponses = allTasks.content.map { task ->
                val leadId = taskToLeadMap[task.id] 
                    ?: throw IllegalStateException("Task ${task.id} is not associated with any lead. Data integrity violation.")
                LeadTasksResponse(
                    leadId = leadId,
                    tasks = listOf(task)
                )
        ...
        return try {
            ...
            val leadTasksResponses = taskData.map { taskDataItem ->
                val task = try {
                    ...
                    throw IllegalStateException("Failed to retrieve task ${taskDataItem.taskId} for lead $leadId: ${e.message}", e)
                }
                LeadTasksResponse(
                    leadId = leadId,
                    tasks = listOf(task)
